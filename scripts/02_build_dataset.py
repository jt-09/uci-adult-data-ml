...
def main():
    ...
    export_table(
        get_cleaning_summary_table(decisions), paths["tables"] / "table_03_cleaning_decisions.csv"
    )
    export_table(
        get_split_summary_table(clean, train_idx, test_idx, cfg),
        paths["tables"] / "table_04_split_summary.csv",
    )
